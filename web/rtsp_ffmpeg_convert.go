...
func GetInfoByRTSP(rtsp string, ip string, port string) StreamInfo {
	key := getRTSPKey(rtsp)
	info := GetInfoByKey(ip, port, key)
	info.Rtsp = rtsp
	return info
}

func GetInfoByKey(ip string, port string, key string) StreamInfo {
	host := getTargetHost()
	if host != "localhost" {
		ip = host
	}
